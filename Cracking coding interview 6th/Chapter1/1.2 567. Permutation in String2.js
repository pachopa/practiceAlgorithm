...
var checkInclusion2 = function (s1, s2) {
  ...
  firstLoop: for (let i = 0; i <= s2Length - s1Length; i++) {
    let slicedString = s2.slice(i, s1Length + i);
    let tempS1 = s1;

    secondLoop: for (let j = 0; j < slicedString.length; j++) {
      if (!tempS1.includes(slicedString[j])) {
        continue firstLoop;
      }
      tempS1 = tempS1.replace(slicedString[j], "");
    }
    return true;
  }
  return false;
